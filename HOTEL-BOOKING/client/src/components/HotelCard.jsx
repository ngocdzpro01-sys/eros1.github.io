...
const HotelCard = ({room, index}) => {
  return (
    <Link to={'/rooms/' + room._id} onClick={()=> scrollTo(0,0)} key={room._id} className='relative max-w-70 w-full
        ...
        <div className='p-4 pt-5'>
            ...
            <div className='flex items-center justify-between mt-4'>
                ...
                <button className='px-4 py-2 text-sm font-bold border
                border-gray-300 rounded hover:bg-gray-50 transition-all
                cursor-pointer'>Đặt Ngay</button>
            </div>
        </div>
    </Link>
  )
